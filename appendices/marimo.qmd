...
## Why Marimo for This Course?

Marimo fits naturally into the workflow this course builds. It has **native SQL cells** that connect directly to DuckDB, so you can write SQL queries alongside Python code without any boilerplate connection logic. It integrates with uv for dependency management. And because notebooks are `.py` files, they work seamlessly with Git, Ruff, and the rest of your quality tooling.
...
Marimo is introduced in @sec-marimo, where you'll use its reactive cells, native SQL integration with DuckDB, and Python cells to build interactive data explorations. The notebook workflow continues through the integration module as you combine SQL, Python, Polars, and Altair in a single environment.
