...
In Unit 1, you used `uv` to manage tools. Now you'll use it to manage Python itself. Install `uv` and Python 3.13 following the instructions in @sec-uv-config. Verify the installation:
...
This file describes your project: its name, version, which Python version it needs, and what external packages it depends on. For now, it's mostly empty. In @sec-python-projects, you'll learn to use `pyproject.toml` extensively. For now, just know that it exists and that `uv` reads it to manage your project.
...
Both forms are identical in behavior. Pick one convention and stick with it. Throughout this book, we'll use double quotes for strings, matching the convention enforced by the Ruff formatter you'll meet in @sec-code-quality.
...
3.3333333333333335
>>> 10 // 3    # Floor division (rounds down to int)
3
...
>>> 10 <= 9
False
>>> 10 == 10   # Equality check, not assignment
True
>>> 10 != 5    # Not equal
True
```
...
**3.** `10 / 5` → `2.0` (float, division always returns float), `10 // 5` → `2` (int), `10 % 5` → `0` (int), `7 % 2` → `1` (int), `2 ** 10` → `1024` (int).

**4.**
```{.python filename="solution.py"}
name = "Northwind Traders"
print("wind" in name)              # True
print(name.startswith("North"))    # True
print(name.endswith("Inc"))        # False
```
...
>>> bool(1)
True
>>> bool(-1)           # Any non-zero number
True
>>> bool(0.001)
True
>>> bool("hello")      # Any non-empty string
True
>>> bool(" ")          # A space is not empty!
True
>>> bool([0])          # A list with one element, even if that element is 0
True
```
...
>>> not False
True
>>> not 0         # 0 is falsy, so not 0 is True
True
>>> not "hello"   # "hello" is truthy, so not "hello" is False
False
>>> not None      # None is falsy
True
```
...
1. `False` (empty list is falsy)
2. `True` (non-empty list, even if elements are 0)
3. `"default"` (empty string is falsy, `or` returns second)
4. `"hello"` (first is truthy, `or` returns it immediately)
5. `0` (`and` returns first falsy value)
6. `"hello"` (`and` returns last value when all truthy)
7. `"finally"` (`or` chains through falsy values until it finds truthy)
8. `True` (`not "hello"` is `False`, `not False` is `True`)
9. `3` (`True` is `1` in arithmetic, so `1 + 1 + 1 = 3`)
10. `""` (`and` returns first falsy, `""` is falsy, stops there before `"world"`)
...
# Convert PSI to kPa using the ISO 80000-4 conversion factor
pressure_psi = 1500
pressure_kpa = pressure_psi * 6.894757  # 1 PSI = 6.894757 kPa
pressure_mpa = pressure_kpa / 1000      # 1 MPa = 1000 kPa
print(f"{pressure_psi} PSI = {pressure_kpa:.2f} kPa = {pressure_mpa:.2f} MPa")

# Convert Fahrenheit to Celsius and Kelvin
temperature_f = 72
temperature_c = (temperature_f - 32) * 5 / 9  # Standard formula
temperature_k = temperature_c + 273.15        # Absolute zero offset
print(f"{temperature_f}°F = {temperature_c:.2f}°C = {temperature_k:.2f}K")

# Convert miles to kilometers and meters (1 mile = 1.60934 km)
miles = 5.5
kilometers = miles * 1.60934
meters = kilometers * 1000
print(f"{miles} miles = {kilometers:.2f} km = {meters:.2f} m")

# Convert pounds-force to Newtons (1 lbf = 4.44822 N)
force_lbf = 2000
force_n = force_lbf * 4.44822
force_kn = force_n / 1000
print(f"{force_lbf} lbf = {force_n:.2f} N = {force_kn:.2f} kN")
```

**REPL Exploration**

```{.python filename="solution.py"}
# 1. What happens when you add a string and an integer?
# TypeError: can only concatenate str (not "int") to str

# 2. What does type(True + 1) return? What does True + 1 evaluate to?
# type(True + 1) → <class 'int'>, evaluates to 2
...
```{.python filename="solution.py"}
def print_product_card(product_name, unit_price, units_in_stock, discontinued, reorder_level):
    """Print a formatted product card."""
    needs_reorder = units_in_stock <= reorder_level and not discontinued
    status = "DISCONTINUED" if discontinued else "Active"
    reorder_flag = " [REORDER NEEDED]" if needs_reorder else ""
    ...
    print(f"Stock Value: ${unit_price * units_in_stock:,.2f}")
    print(f"{'=' * 50}\n")


# Chai
print_product_card("Chai", 18.00, 29, False, 48)

# Chang
print_product_card("Chang", 19.00, 68, False, 46)

# Chef Anton's Gumbo Mix
print_product_card("Chef Anton's Gumbo Mix", 21.35, 45, True, 37)
```
