...
Throughout this module, we'll work with the **Northwind** database, a dataset representing a fictional company called Northwind Traders that imports and exports specialty food products worldwide. Originally created by Microsoft as a teaching database, Northwind has become one of the most widely-used sample datasets in the SQL world, and for good reason: it models a realistic supply chain with suppliers, products, inventory, customer orders, shipping, and employees.

The Northwind database contains the following tables, each representing an entity in the business:

| Table | What It Represents | Example Record |
|---|---|---|
| `categories` | Product classifications | "Beverages", "Condiments" |
| `suppliers` | Companies that supply products | "Tokyo Traders", Japan |
| `products` | Items Northwind sells | "Chai", $18.00, 39 in stock |
| `customers` | Companies that buy from Northwind | "Bottom-Dollar Markets", Canada |
| `employees` | Northwind staff who process orders | "Davolio, Nancy", Sales Rep |
| `orders` | Customer purchase transactions | Order #10248, July 4 1996 |
| `order_details` | Line items within each order | 12 units of "Queso Cabrales" at $14 |
| `shippers` | Delivery carriers | "Federal Shipping" |

::: {.callout-note}
## Downloading the Data
The Northwind CSV files are available on this book's website under the data downloads section. Download them into a `data/northwind/` directory in your working folder before proceeding. If you're following along in the DuckDB UI, make sure you launched DuckDB from the directory containing your `data/` folder so the relative paths resolve correctly.
:::
...
CREATE TABLE categories AS FROM read_csv('data/northwind/categories.csv');
CREATE TABLE suppliers AS FROM read_csv('data/northwind/suppliers.csv');
CREATE TABLE products AS FROM read_csv('data/northwind/products.csv');
CREATE TABLE customers AS FROM read_csv('data/northwind/customers.csv');
CREATE TABLE employees AS FROM read_csv('data/northwind/employees.csv');
CREATE TABLE orders AS FROM read_csv('data/northwind/orders.csv');
CREATE TABLE order_details AS FROM read_csv('data/northwind/order_details.csv');
CREATE TABLE shippers AS FROM read_csv('data/northwind/shippers.csv');
```

Notice a few things here. `CREATE TABLE ... AS` creates a new table from the results of a query. `read_csv()` is a DuckDB function that reads a CSV file and infers column names and types automatically. The `FROM` keyword without `SELECT` is a DuckDB convenience, shorthand for `SELECT * FROM`, which we'll discuss in the next chapter.
...
FROM read_csv('data/northwind/products.csv') LIMIT 5;
```
