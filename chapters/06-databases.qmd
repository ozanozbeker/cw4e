...
Install DuckDB using your package manager (see @sec-duckdb-config). Verify the installation:
...
Download `northwind.duckdb` from @sec-northwind-dataset and save it to a dedicated folder like `~/cw4e`. Navigate to that directory, verify the file is there with `ls`, then open the database:
...
::: {.callout-warning}
## DuckDB Creates Missing Files Silently
If you provide a filename that doesn't exist, DuckDB silently creates an empty database instead of warning you. If `SHOW TABLES` returns nothing, you likely opened the wrong file or are in the wrong directory. Use `ls` and `pwd` to verify your location before launching DuckDB. See @sec-northwind-dataset for detailed troubleshooting.
:::
...
You'll see columns like `product_name` with type `VARCHAR` (text), `unit_price` with type `DOUBLE` (numbers), and `discontinued` with type `BIGINT` (whole numbers). These **data types** determine what operations are valid on each column: you can calculate an average of `DOUBLE` values but not of `VARCHAR` values. We'll explore data types more in @sec-modeling, but for now, knowing how to inspect what's in a database is the important skill.
...
5. **Predict and verify.** Before running each of the following in DuckDB, predict what will happen, then run them and check your prediction.
    ...
    C.  `SELECT * FROM employees WHERE reports_to IS NULL;` followed by `SELECT * FROM employees WHERE reports_to = NULL;`
