...
3. Write a function `format_product(name, price, *, currency="$", decimals=2)` where `currency` and `decimals` are keyword-only arguments. Call it three different ways: with defaults, with a custom currency, and with zero decimal places.

4. What happens if you call `create_product("Chai", 18.00, "Beverages")` vs. `create_product(price=18.00, name="Chai", category="Beverages")` for a function with positional parameters `(name, price, category)`? Are the results the same?
...
3. A colleague writes `list = [1, 2, 3]` at the top of their script. Later, `list(range(5))` fails with a `TypeError`. Explain what went wrong and how to fix it.
...
3. Explain why `sum(n ** 2 for n in range(1_000_000))` uses almost no memory, while `sum([n ** 2 for n in range(1_000_000)])` uses significant memory. Which should you prefer?
...
**1.**
```{.python filename="solution.py"}
def classify_stock_level(units_in_stock, reorder_level):
    """Classify inventory stock level based on reorder threshold.

    Args:
        units_in_stock: Current number of units in inventory.
        reorder_level: The target minimum stock level.

    Returns:
        A classification string: "Critical", "Low", "Adequate", or "Overstocked".
    """
    if units_in_stock == 0:
        return "Critical"
    elif units_in_stock <= reorder_level:
        return "Low"
    elif units_in_stock < 3 * reorder_level:
        return "Adequate"
    else:
        return "Overstocked"

# Test cases
print(classify_stock_level(0, 10))      # "Critical"
print(classify_stock_level(5, 10))      # "Low"
print(classify_stock_level(15, 10))     # "Adequate"
print(classify_stock_level(35, 10))     # "Overstocked"
```

**2.**
```{.python filename="solution.py"}
def format_product_line(name, price, quantity):
    """Format a product line with alignment for display.
    ...
    Returns:
        A formatted string like 'Chai ............. $18.00 x 10'.
    """
    dots = "." * (20 - len(name))
    return f"{name} {dots} ${price:>7.2f} x {quantity:>3}"

print(format_product_line("Chai", 18.00, 10))
print(format_product_line("Aniseed Syrup", 10.00, 20))
print(format_product_line("Tofu", 23.25, 5))
```

**3.**
```{.python filename="solution.py"}
def group_orders(order_details):
...
def compute_category_totals(order_details):
    """Compute revenue totals by category."""
    groups = group_orders(order_details)
    totals = {}
...
totals = compute_category_totals(order_details)
...
# Generator expression for square roots
sqrt_gen = (math.sqrt(n) for n in range(1, 1_000_001))

# Sum without creating a list
total = sum(sqrt_gen)
print(f"Sum of square roots: {total:.2f}")
```
...
# List comprehension (uses significant memory)
list_comp = [n ** 2 for n in range(10_000_000)]
list_size = sys.getsizeof(list_comp)

# Generator expression (uses almost no memory)
gen_expr = (n ** 2 for n in range(10_000_000))
...
print(f"The list comprehension uses ~{list_size / gen_size:.0f}x more memory")
```
...
File: `conversions.py`
```{.python filename="conversions.py"}
"""Unit conversion utilities for engineering calculations.

Provides functions for common unit conversions used in industrial
engineering, including pressure, temperature, and distance.
"""
...
if __name__ == "__main__":
    print(f"100 PSI = {psi_to_kpa(100):.2f} kPa")
    print(f"72°F = {fahrenheit_to_celsius(72):.2f}°C")
    print(f"10 miles = {miles_to_kilometers(10):.2f} km")
```

File: `use_conversions.py`
```{.python filename="use_conversions.py"}
"""Example script using the conversions module."""

from conversions import psi_to_kpa, fahrenheit_to_celsius, miles_to_kilometers
...
print(f"{pressure} PSI = {psi_to_kpa(pressure):.2f} kPa")
print(f"{temperature}°F = {fahrenheit_to_celsius(temperature):.2f}°C")
print(f"{distance} miles = {miles_to_kilometers(distance):.2f} km")
```

**2.**

Add this function to `northwind_utils.py`:

```{.python filename="solution.py"}
def top_products_by_value(products, n=5):
    """Return the top N products by stock value (price × quantity).

    Args:
        products: List of product dictionaries, each with 'name',
            'price', and 'stock' keys.
        n: Number of top products to return. Defaults to 5.

    Returns:
        A list of the top N products sorted by stock value (descending),
        each as a dictionary with 'name' and 'value' keys.
    """
    products_with_value = [
        {
            "name": p["name"],
