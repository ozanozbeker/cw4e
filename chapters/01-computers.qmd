...
::: {.callout-note collapse="true" title="Exercise: RAM vs. Disk Performance"}
...
::: {.callout-note collapse="true" title="Exercise: Predicting Memory Behavior"}
...
::: {.callout-note collapse="true" title="Exercise: The Two Lives of a File"}
...
For this book, we'll use **Zed**, a modern text editor designed for professional programming work (see @sec-zed-config for installation and features). Zed provides syntax highlighting, auto-completion, an integrated terminal, and unsaved change indicators. When I refer to "the command line" in later chapters, you can use Zed's integrated terminal instead of a separate terminal window.
...
For **Windows**, use **WinGet** (see @sec-winget-config). For **macOS**, use **Homebrew** (see @sec-brew-config). Throughout this book, when we introduce new tools, I'll provide package manager commands as the recommended installation method. Getting comfortable with your package manager early makes everything else smoother.
...
**1.1** B) Applications are designed for end-user interaction with thoughtful interfaces

**1.2** C) RAM is where programs run and files are actively edited

**1.3** B) The file must be copied from disk into RAM before work can begin

**1.4** C) They are lost because they only existed in volatile RAM

**1.5** B) Word embeds hidden formatting codes that confuse programming languages

**1.6** B) The memory contents are lost when power is removed

**1.7** B) Package managers automatically handle dependencies when installing software

**1.8** B) A program written in an interpreted language like Python

**1.9** B) Colors different parts of code based on their grammatical role
