...
- [northwind.duckdb](../../data/northwind.duckdb){download="northwind.duckdb"}
- [northwind.sqlite](../../data/northwind.sqlite){download="northwind.sqlite"}

**Parquet**

- [northwind_flat.parquet](../../data/northwind_flat.parquet){download="northwind_flat.parquet"}
...
- [data_dictionary.csv](../../data/data_dictionary.csv){download="data_dictionary.csv"}
:::
...
The CSV files use **camelCase** column names (`orderID`, `unitPrice`, `companyName`) matching the original Microsoft Northwind convention. The DuckDB and SQLite databases use **snake_case** column names (`order_id`, `unit_price`, `company_name`), which is the modern convention preferred by most data teams. The denormalized Parquet file also uses snake_case, with table-prefixed names where needed to avoid ambiguity (e.g., `customer_city` vs. `supplier_city`). Code examples throughout the book use whichever convention matches the file format being used in that chapter.
:::
...
- [shippers.csv](../data/shippers.csv)
- [suppliers.csv](../data/suppliers.csv)
