...
Every data workflow touches files. Databases store data in files. CSV exports are files. JSON configuration is a file. The reports you generate are files. In the Foundations module, you learned to navigate files through the command line. Now you'll learn to read, write, and manipulate them from Python.
...
Context managers aren't limited to files. You'll encounter them with database connections (in the Integration module), network requests, and many other resources that need setup and teardown. The `with` pattern is one of Python's most important idioms.
...
p.exists()      # True/False: does this path exist?
p.is_file()     # True/False: is it a file?
p.is_dir()      # True/False: is it a directory?
p.name          # "orders.csv" (filename with extension)
p.stem          # "orders" (filename without extension)
p.suffix        # ".csv" (file extension)
p.parent        # Path("data") (the containing directory)
p.resolve()     # Absolute path: /home/user/project/data/orders.csv
```
