...
6. **Category revenue report.** Write a query that answers: "What is the total revenue for each product category?" You'll need to join `categories`, `products`, and `order_details`. Then extend it to show the percentage of total company revenue that each category represents. (Hint: you can compute the overall total using a subquery.)
