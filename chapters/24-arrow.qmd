...
In the Integration module, something remarkable happened without explanation. You converted a DuckDB query result to a Polars DataFrame with `.pl()`, and it was essentially instant. You passed a Polars DataFrame into a DuckDB SQL query as a table name, and it just worked. Altair accepted Polars DataFrames directly without any conversion step. A 200,000-row dataset flowed between three different tools with no perceptible delay.

*How?*

The answer is Apache Arrow, a shared in-memory data format that all three tools speak natively. This chapter reveals the hidden infrastructure that made your Integration workflows fast and seamless. It then broadens the picture to explain how Python connects to databases beyond DuckDB, tracing the evolution from ODBC in the 1990s to ADBC (Arrow Database Connectivity), the modern standard that brings Arrow's performance to any database connection.
...
Parquet, which you've used throughout this course for the Northwind flat file, is essentially Arrow's on-disk cousin. Parquet stores data in a columnar format on disk; Arrow stores it in a columnar format in memory. Reading a Parquet file into Polars is efficient because the formats are closely aligned.
...
This course doesn't teach SQLAlchemy in depth for two reasons. First, it's a large library that solves problems in web application development, a domain different from analytical data engineering. Second, it abstracts away the SQL you just spent a module learning. But you should recognize it when you encounter it, because you will.
...
For the analytical work you're doing in this course, DuckDB handles most needs. ADBC becomes relevant when you need to connect to a PostgreSQL or Snowflake server to pull data for analysis. SQLAlchemy is for a different world (web applications), but you should recognize it.
