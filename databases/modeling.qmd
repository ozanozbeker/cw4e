...
Pick a naming convention and enforce it ruthlessly: `snake_case` or `camelCase` for columns, singular or plural table names, whether IDs are `id`, `product_id`, or `productID`. It seems trivial until you inherit a database where every table uses a different convention and you can't remember whether the column is `customerId`, `customer_id`, or `cust_id`. The Northwind database uses `camelCase` (e.g., `unitPrice`, `companyName`), which is common in Microsoft ecosystems. Most modern data teams prefer `snake_case` (`unit_price`, `company_name`) because it's easier to read. Whatever you choose, consistency is more important than the choice itself.
:::
...
Use `DECIMAL` for money, never `DOUBLE`. Floating-point arithmetic introduces rounding errors that accumulate in financial calculations. `DECIMAL(10, 2)` stores values like `1234567.89` exactly as specified. You may have noticed that the Northwind database stores `unitPrice` as `DOUBLE`. This is a common shortcut in sample datasets, but in a production system handling real financial data, you'd define it as `DECIMAL(10, 2)` to guarantee exact arithmetic.
...
SELECT p.productName, c.categoryName, s.companyName AS supplier,
    p.unitsInStock AS current_stock, p.reorderLevel
FROM products AS p
INNER JOIN categories AS c ON p.categoryID = c.categoryID
INNER JOIN suppliers AS s ON p.supplierID = s.supplierID
...
SELECT o.orderID, c.companyName, o.orderDate, COUNT(od.productID) AS line_items,
...
SELECT s.companyName, COUNT(DISTINCT p.productID) AS product_count,
    ROUND(AVG(p.unitPrice), 2) AS avg_price,
