...
This is the same concept you learned in @sec-cli: programs look for files relative to the directory they were launched from. When you type `duckdb northwind.duckdb`, DuckDB looks for `northwind.duckdb` in your current directory, just like `ls` lists the contents of wherever you are right now. If DuckDB can't find the file, the first thing to check is whether you're in the right place.

::: {.callout-warning}
## A Common Pitfall
If you see an error like `IO Error: No such file or directory`, DuckDB can't find the database file. This almost always means you launched DuckDB from a different directory than where the file lives. Close DuckDB, `cd` to the correct directory, and try again.

:::

### Opening the Database

Navigate to the directory where you saved the file and launch DuckDB with it:
...
You'll see columns like `productName` with type `VARCHAR` (text), `unitPrice` with type `DOUBLE` (decimal numbers), and `discontinued` with type `BIGINT` (whole numbers). These **data types** determine what operations are valid on each column: you can calculate an average of `DOUBLE` values but not of `VARCHAR` values. We'll explore data types more in @sec-modeling, but for now, knowing how to inspect what's in a database is the important skill.
