...
- **Ruff**: A Python linter and formatter that enforces code style and catches bugs automatically. Zed runs it out of the box; added as a project dependency in @sec-code-quality. See @sec-ruff-config.
- **basedpyright**: A static type checker that verifies your type hints before you run your code. Zed runs it out of the box; added as a project dependency in @sec-code-quality. See @sec-basedpyright-config.
...
**When you need it:** @sec-quarto (Writing with Quarto), and again in @sec-computational-documents for executable code cells.
...
**Notes:** DuckDB can query CSV, Parquet, and JSON files directly, making it ideal for learning SQL without database administration overhead. You'll later add `duckdb` as a Python library (`uv add duckdb`) when connecting SQL to Python in @sec-python-sql. See @sec-duckdb-config for the distinction between the CLI tool and the Python library.
...
**When you need it:** @sec-python-fundamentals (Python Fundamentals) through @sec-objects (The Python Object Model).
...
**When you need it:** @sec-computational-documents (Computational Documents).
...
4. If you're working through this book with others, ask for help and bring the exact error message.
